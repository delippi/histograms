...
#colors_list = list(mcolors.cnames)
#print(colors_list)
...
for i in range(buf): 
   if(i%8 == 0):
      ...
      j=0
   y_series[buf]=np.array(y_series[buf])
   plt.plot(x_series,y_series[i],color=colors[i],linewidth=width,label=bufr_types[i])
axs.legend(ncol=2)
plt.xticks(x_series)
...
plt.show()
#plt.savefig("./inventory_%s_%s_%s_to_%s_%sz.png" % (str(bufr_types[0]),str(bufr_types[-1]),str(start),str(end),str(cyc).zfill(2)))
