program bufrhist
  ...
  integer(i_kind) :: iret,idate,ireadmg,ireadsb,num_message,num_subset,tm,tmm,anl_time,dne
  ...
  character(150)  :: bufrfilename
  character(80)   :: hdstr,hdstr_prep
  real(r_kind)    :: hdr(10)
  real(r_kind)    :: dhr,hour
  character(8)    :: subset
  character(22),dimension(48) :: bufr_types
  !character(22),dimension(1) :: bufr_types
...
!PATHS ON THEIA
!path="/scratch4/NCEPDEV/fv3-cam/noscrub/Donald.E.Lippi/hrlyGDAS/" !theia
!path2="/scratch3/BMC/gsienkf/whitaker/hrlygdas/rotdir/"           !theia
!PATHS ON HERA
path="/scratch2/NCEPDEV/fv3-cam/Donald.E.Lippi/hrlyGDAS/" !hera
path2="/scratch2/BMC/gsienkf/whitaker/hrlygdas/rotdir/"   !hera
...
!Comprehensive list of bufr types (48 total).
...
!bufr_types=(/"prepbufr.acft_profiles"/)
tm=0
dne=0
do buf = 1,size(bufr_types) !loop over bufr types
   ...
   if(trim(bufr_types(buf)) == "prepbufr" .or. &
      trim(bufr_types(buf)) == "prepbufr.acft_profiles") then
      bufrfilename=trim(path2)//'/gdas.'//pdy//'/'//cyc//'/gdas.t'//cyc//'z.'//trim(bufr_types(buf))//'.uniq'
   else
   ...
   inquire(file=bufrfilename,exist=file_exists)
   if(.not. file_exists) then
      dne=dne+1
      if(dne==1) then
        open(unit=25,file="./missing_files",status='unknown')
        write(25,'(a34)') "The following files did not exist:"
      endif
      write(6,*) "Does not exist: ",bufrfilename
      write(25,'(a120)') bufrfilename
      cycle
   endif
